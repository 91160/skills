...
function init() {
  ...
  if (fs.existsSync(AGENTS_FILE)) {
    ...
    console.log(gray('     如需更新请使用: npx skills update\n'))
  } else {
  ...
  if (!toolsValue) {
    ...
    console.log('')
    console.log(gray('  示例: npx skills init --tools=A'))
    console.log('')
  }
...
switch (command) {
  ...
  case undefined:
    printBanner()
    console.log(bold('  用法：') + 'npx skills <command>\n')
    console.log(bold('  命令：'))
  ...
  default:
    ...
    console.log('  运行 npx skills --help 查看可用命令')
    process.exit(1)
}
