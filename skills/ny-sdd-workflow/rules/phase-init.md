...
3. Skill 文件缺失 → 使用内置兜底：读取 {SKILL_DIR}/rules/fallback/frontend-scan.md，按其指令扫描，产出直接写入 .project/specs/rules/frontend-context.md
4. 必须输出 Skill 执行日志
```
...
- `.docs/prd/` — PRD 文字需求文档（md/pdf/txt）
- `.docs/prd/prototype/` — 原型图（线框图 png/jpg/pdf）
- `.docs/prd/ui/` — UI 设计稿（高保真视觉 png/jpg/pdf）
- `.docs/prd/ui-spec/` — UI 解析文件（从 Figma/蓝湖等工具导出的结构化 md）
- `.docs/tech/` — 技术文档（API 文档、建表脚本、中间件配置、对接手册等）

文件格式和命名不限，可以多个文件也可以全部放一个文件。**子目录不强制**，放在 `.docs/prd/` 根目录的文件 AI 会自动识别归类。
...
1. 按顶级目录分类处理：
   - `.docs/prd/` → **全部文件均为需求内容**，按下方「PRD 内容扫描」规则分类处理
...
**PRD 内容扫描**（`.docs/prd/` 分类处理规则）：

`.docs/prd/` 下文件分为**四类内容**，AI 按「子目录 → 文件名关键词 → 内容视觉判断」三级策略识别：

| 内容类别 | 子目录（优先识别） | 根目录识别规则（兜底） | 提取重点 |
|---------|---------------------|-----------------------|---------|
| **需求文档** | 根目录 `*.md` / `*.pdf` / `*.txt` | `*.md/*.txt` → 默认归此类 | 业务逻辑 / 验收标准 / 异常场景 |
| **原型图** | `prototype/*.png/jpg/pdf` | 视觉：线框/简化/黑白灰风格 | 交互流程 / 页面跳转 / 布局骨架 |
| **UI 设计稿** | `ui/*.png/jpg/pdf` | 视觉：高保真/有色彩/真实 UI 元素 | 视觉规范（颜色/字号/间距）/ 组件布局 |
| **UI 解析 md** | `ui-spec/*.md` | `*.md` 文件名含 `ui-spec` / `spec` / `标注` | 精确参数（像素值/组件名/字段清单/状态标注） |

**视觉内容判断无法确定时**：保守归为"UI 设计稿"类（较高保真路径，避免丢失视觉信息）。

解析后按类别分别处理：

1. **建索引**：所有文件写入 project-profile.md「PRD 内容索引」区块（含「内容类别」列）：
   ```
   ## PRD 内容索引
...
2. **图片/解析 md 落入 REQ**：
   - 原型图解析 → 交互流程和页面跳转写入 REQ 的"业务逻辑"部分
   - UI 设计稿解析 → UI 需求（字段清单/状态/布局）写入 REQ 的"UI 需求"部分
   - UI 解析 md → 精确参数（像素值/组件名）在 §2.3 DES 前端方案中引用

3. **后续阶段加载方式**：各阶段通过索引定位源文件，**重新读取原始文件**（不依赖记忆），编码时 UI 基准优先级见 §3.1.4
