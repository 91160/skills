...
> 本文件在项目初始化阶段加载。完成后进入 `{SKILL_DIR}/rules/phase-spec.md` 或 `{SKILL_DIR}/rules/phase-coding.md`。
...
AI 检查 .docs/prd/ 是否有文件：
  ...
  → AI 执行 PRD 审计（必须输出 Skill 执行日志）：
    ...
    3. P0 必须解决后才继续
  → AI 从 PRD 提取业务流程，首次生成 .outdocs/project-overview.md，写入「二、业务架构」「三、核心业务流程」部分（新项目不写入 project-profile.md，待需求澄清完善后再沉淀；其余章节留空，由 §2.0 补充）
  → AI 扫描 .docs/ 目录，按下方「技术文档处理规则」处理：
    - .docs/prd/ 有文件 → 已在上方读取，此处跳过
    - .docs/tech/ 有文件 → 按技术文档流程处理，扫描到的信息（技术栈/外部依赖/数据库等）直接采用，后续询问时跳过已知项
    - .docs/tech/ 为空 → 跳过（§0 第二步半已确认过，不再重复询问）
  ...
  → 安装依赖，确认能跑起来
  → 执行「前后端规范提取」（见下方）
  → 读取 `{SKILL_DIR}/rules/phase-spec.md`，进入 §2.0 任务拆分（全量模块排序 + 全量模块子任务拆分）
  → 进入图二业务开发循环
```
...
- 扫描 .docs/ 目录，按下方「技术文档处理规则」处理（§0 第二步半已确认过文档补充，此处直接扫描，不再重复询问）
...
- 执行「前后端规范提取」（见下方）
- 执行「深度业务代码扫描」（见下方）→ 成功则跳过兜底，失败则执行兜底
- **兜底：业务架构与流程提取**（仅在深度扫描 Skill 不可用时执行）→ 写入 project-profile.md「业务架构」「业务流程」区块，人工确认
  ...
  **兜底深度标准**：扫描粒度为路由/控制器 + 服务层入口方法，不要求逐函数级别（那是 reverse-scan 的职责）。目标是建立模块级认知，能支撑 §2.0 任务拆分。
...
AI 生成 .project/ 目录（汇总所有信息）
  ...
  → feature 类型 → 检查 .docs/prd/ 是否有文件，有则执行全量 PRD 审计（与 §1.1 一致）：
      ...
      3. P0 问题必须解决后才继续
    → 读取 `{SKILL_DIR}/rules/phase-spec.md`，进入 §2.0 任务拆分（全量模块排序 + 拆分，与新项目一致）
  → bug / refactor 类型 → 跳过 §2.0，直接读取 `{SKILL_DIR}/rules/phase-spec.md`，进入 §2.1
  → 进入图二业务开发循环
```
...
- 每次标准通道和快速通道开始前必须先读取铁律（执行详见 phase-coding.md Step 0）
- 未收到用户明确指示不得继续执行
...
> 在前后端规范提取完成后执行。通过逐文件逐函数级别的深度扫描，自底向上建立代码认知层，为后续编码提供精准的复用参考。
...
| 产出 | 用途 | 后续谁消费 |
|------|------|-----------|
| `knowledge-cards/*.md` | 逐文件函数级知识卡片 | §2.1 需求分析(feature/refactor) / §2.2 方案设计 / §3 Step 0,3,4,5 编码与审计 |
| `call-graph.md` | 全局调用关系图 | §2.1 需求分析(refactor) / §2.2 方案设计 / §3 Step 3 影响面评估 |
| `module-map.md` | 已有模块划分 + 职责 + 依赖 | §2.0 任务拆分 |
| `db-schema.md` | 完整数据库结构 | §2.2 数据模型设计 / §3 Step 2 文档学习 / §3 Step 3 影响面评估 |
| `specs/design/DES-*.md` | 已有模块的设计规格（逆向） | §2.2 参考已有技术方案 / §3 Step 0 编码上下文加载 |
| `specs/requirements/REQ-*.md` | 已有模块的需求规格（逆向） | §2.1 界定新旧功能边界 |
...
2. **图片解析落入 REQ**：图片解析出的结构化信息（布局/组件/字段/交互/状态），在 §2.1 写 REQ 时直接写入验收标准的「UI 需求」部分（不仅是索引摘要）
