...
| 15 | 单元测试已通过 | 标准通道 | 检查 `.test/unit/report.md`：结论 PASS → ✅；结论 SKIP → ✅（已降级，§3.7.2 全量验证已补充覆盖）；结论 FAIL 或文件不存在 → 回 §3.7 |
...
**feature 类型**：根据执行模式（§2.1 Step 4 选择）衔接下一模块：
  ...
  **连续模式**：自动进入 dev-order 下一轮模块，输出简要通知后直接开始：
  ...
  （E2E 测试消费 §2.7 产出的功能测试用例，执行时间较长）
    A. 执行 E2E 测试（调用 e2e-test-skill，当前未发布则跳过）
    B. 跳过（不执行 E2E 测试）
  ```
