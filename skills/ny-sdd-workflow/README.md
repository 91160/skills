...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.0.0 | 2026-04-17 | 首次发布。G 系列全局规则（G0 对话初始化 / G1 写码门禁 / G2 停车信号 / G3 未覆盖场景兜底）+ §1~§4 阶段编号 + 流程声明头机制（phase/step/prev/next/gate/blocking）+ 动态加载架构 + PRD 三类内容管理（prototype/ui/ui-spec）+ UI 基准 6 级优先级 + context.md 状态标记规约 + 10 条编码规约 C-01~C-10 + 6 条 UI 还原规约 U-01~U-06 + 7 条审计规约 S-01~S-07 + 6 条自测规约 T-01~T-06 + 多 AI 工具兼容（8 款）|
