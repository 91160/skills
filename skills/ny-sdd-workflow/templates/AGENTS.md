---
inclusion: always
description: "SDD 开发工作流 v3.3 — 写码门禁 + Skill路由 + Skill执行流程 + 技术文档 + 任务拆分 + task管理 + 需求变更管理 + 交付文档 + 人工批注同步 + 任务分类 + PRD审计 + 编码规约 + 代码审计 + 开发自测"
---
# SDD Workflow — AI 执行规则 v3.3
...
4. AI 判断是否涉及多步骤，如涉及则拆子任务并写入 task.md，询问用户确认：
...
- **task.md 校验（feature 类型，或 refactor 有 task.md 时）**：归档前校验 task.md 当前模块所有子任务 status 均为 `done`
...
- 按融合规则与 project-profile.md 融合，Skill 不可用则由用户确认是否使用内置规约
- 融合优先级：project-profile.md > Skill 规范 > S-01~S-05
...
*SDD Workflow v3.3 · 2026-04 — v3.2基础上：新增 task.md 全局任务管理（§2.0 全量拆分 + §3 编码联动 + §2.8 归档校验）、project-profile.md 三区重组（一、项目级 + 二、前端 + 三、后端）+ 内部公共能力体系化（前端 8 子节 + 后端 9 子节）+ 编码上下文按任务类型加载、交付文档合并为 project-overview.md（含结构模板 §4.1）、.docs/ 分 prd/tech 子目录、vant-vue3 Skill 仓库独立*
