---
inclusion: always
description: "SDD 开发工作流 v3.0 — 写码门禁 + Skill路由 + Skill执行流程 + 技术文档 + 任务拆分 + 需求变更管理 + 交付文档 + 人工批注同步 + 任务分类 + PRD审计 + 编码规约 + 代码审计 + 开发自测"
---
...
AI 生成 .project/ 目录（汇总所有信息）
  → 安装依赖，确认能跑起来
  → feature 类型 → 进入 §2.0 任务拆分（单模块拆分，跳过排序）
...
用户将技术文档（API 文档、建表脚本、中间件配置、对接手册等）放入项目根目录 `.docs/`，文件格式和命名不限，可以多个文件也可以全部放一个文件。
...
1. 读取 `.docs/` 下所有文件
2. 自动识别内容类别（API 接口 / 数据库结构 / 中间件配置 / 其他）
3. 在 project-custom.md「外部依赖」区块建立索引（内容摘要 + 来源文件路径）
...
- §1.2 旧项目新增需求（仅 feature 类型） → 单模块拆分，跳过排序
- 用户主动要求重新规划时
- bug / refactor 类型 → **跳过 §2.0**，直接进入 §2.1
...
用户确认排序后，对第 1 轮模块按以下维度拆分（后续模块进入时再拆）：
...
**Step 3：写入 index.md + REQ**

用户确认后：

- index.md 填入 `depends` 和 `dev-order` 列的值，记录依赖关系和开发顺序
- 子任务清单写入对应模块的 REQ 文件，作为验收清单的一部分

> 后续模块进入 §2.1 时，AI 自动执行该模块的 Step 2 拆分（如尚未拆分）。
...
根据任务类型，使用对应的 DES 模板，写入 `.project/specs/master/design/DES-{xx}-{name}.md`：
...
- **C. 技术文档变更**：用户说"文档更新了" / ".docs 有新文件" / "我加了文档" / "API 文档改了" / "建表脚本更新了" / "接口变了" / "数据库结构改了" → 先执行 Step 0.5，再进入 Step 1
...
Step 0: PRD 变更识别（仅触发源 B 执行）
  → AI 重读 PRD 变更内容，判断变更类型：
    a. 新增模块：
       ...
       - 拆子任务（按 §2.0 Step 2 维度）
       - 创建 REQ + DES 文件
    b. 已有模块变更：
       - 定位受影响的 REQ + DES
       - 更新 REQ（需求描述、验收标准）
       - 更新 DES（技术方案、接口设计）
       - 可能需要重新拆子任务（REQ 中追加新子任务）
  → 输出变更摘要，用户确认后
...
- **变更内容**：{具体改了什么}
- **影响模块**：{受影响的模块编号和名称}
- **影响范围**：{REQ/DES/代码/api-doc/dev-order 哪些受影响}
...
- 生成任务执行摘要：汇总 REQ、子任务完成情况、接口清单、代码变更概要，以模块章节形式追加到 `.outdocs/task-report.md`
...
优先从 `.docs/` 中查找当前任务涉及的技术文档（按 §1.4 规则）。`.docs/` 未覆盖且涉及未记录的 API / 库时触发：

```
【文档学习确认】
本次任务涉及 {API/库名}，是否有文档需要我先学习？
支持：A. 在线链接  B. PDF  C. 粘贴内容  D. 放入 .docs/ 目录
```
...
├── .docs/                             ← 用户提供的技术文档，格式命名不限（§1.4）
│   └── *.md / *.sql / *.pdf / ...     ← API文档/建表脚本/中间件配置/对接手册等，AI 自动识别分类
...
*SDD Workflow v3.1 · 2026-03 — v3.0 基础上：新增 §1.4 技术文档（.docs/ 自动扫描+索引+编码时以文档为准）*
