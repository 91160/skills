...
先读取 project-profile.md：一、项目级的「业务架构」「业务流程」（仅旧项目有），以及按任务类型读取对应区域（前端 → 二、前端；后端 → 三、后端）的「项目架构」「构建与运行」，确保方案与现有架构模式、数据库访问方式、路由注册方式一致，且不破坏现有业务流程。然后根据任务类型，使用对应的 DES 模板，写入 `.project/specs/master/design/DES-{xx}-{name}.md`：
...
│   │   └── rules/
│   │       ├── project-profile.md      ← 三区结构：一、项目级（铁律/技术栈/外部依赖/业务架构/业务流程）二、前端（构建/架构/规范/内部公共能力）三、后端（构建/架构/规范/内部公共能力）
...
*SDD Workflow v3.3 · 2026-04 — v3.2基础上：新增 task.md 全局任务管理（§2.0 全量拆分 + §3 编码联动 + §2.8 归档校验）、project-profile.md 三区重组（一、项目级 + 二、前端 + 三、后端）+ 内部公共能力体系化（前端 8 子节 + 后端 9 子节）+ 编码上下文按任务类型加载、交付文档合并为 project-overview.md（含结构模板 §4.1）*
